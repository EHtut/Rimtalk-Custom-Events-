...
| `TalkRequest.IsExpired()` → non-user requests expire after `GetTicksForDuration(20)` | `Data/TalkRequest.cs:44` | Why beats are queued as User: they must not evaporate while a pawn is briefly busy. |
...
| `RimTalkPromptAPI.InjectPawnSection(modId, name, anchor, position, provider)` | `API/RimTalkPromptAPI.cs` | Official extension surface. This is how a CONTINUE **modifier** reaches every prompt for a pawn. Anchors are nested: `ContextCategories.Pawn.Health`, not `ContextCategories.Health`. |
| `TalkType.User` requests are drained **every second** by their own loop, straight to `GenerateTalk` for that pawn | `Patch/TickManagerPatch.cs:73-95` | ⚠️ **This is the channel beats use.** `TalkType.Event` instead waits for RimTalk's own pawn selector to pick your pawn, sits behind the talk-interval and busy gates, and expires in ~20s. User requests never expire and skip `AllowMonologue`. |
